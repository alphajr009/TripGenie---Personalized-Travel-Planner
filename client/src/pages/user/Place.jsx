...
function Place() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = (
          await axios.post("/api/places/getplacebyid", {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = (
          await axios.post("/api/reviews/getreviewbyid", {
            placeid: params.placeid,
          })
        ).data;
    ...
    const checkSavedStatus = async () => {
      ...
      if (user) {
        ...
        try {
          const {
            data: { hasSaved },
          } = await axios.post("/api/users/check-save", {
            placeId: params.placeid,
            userId,
          });
          setSaved(hasSaved);
    ...
    const checkLikedStatus = async () => {
      ...
      if (user) {
        ...
        try {
          const {
            data: { hasLiked },
          } = await axios.post("/api/places/check-like", {
            placeId: params.placeid,
            userId,
          });
          setLiked(hasLiked);
  ...
  useEffect(() => {
    ...
    const fetchDataAndCheckSavedStatus = async () => {
      try {
        const data = await axios.post("/api/users/getuserbyid", {
          userid: user._id,
        });
        setPlace(data.place[0]);
        setSearch(data.place[0].city);
  ...
  const handleSaveButtonClick = async () => {
    ...
    try {
      ...
      if (saved) {
        ...
        await axios.post("/api/users/unsave", {
          placeId: params.placeid,
          userId,
        });
      } else {
      ...
      <div>
        <div>
          <img
            ...
            src={`/uploads/${params.placeid}-0.jpg`}
            alt=""
          />
        </div>
        ...
        <div className="place-cover-save">
          <a onClick={() => handleSaveButtonClick()}>
            {saved ? (
              <FireFilled className="saved-fire" />
            ) : (
              <FireOutlined className="size-fire" />
            )}
          </a>
        </div>
            ...
            <div className="frameDiv">
              <div className="group">
                ...
                <img
                  className="icon2"
                  alt=""
                  src={`/uploads/${params.placeid}-1.jpg`}
                />
                <img
                  className="icon3"
                  alt=""
                  src={`/uploads/${params.placeid}-2.jpg`}
                />
              </div>
              <img
                className="icon4"
                alt=""
                src={`/uploads/${params.placeid}-3.jpg`}
              />
              <img
                className="icon5"
                alt=""
                src={`/uploads/${params.placeid}-4.jpg`}
              />
              <img
                className="icon6"
                alt=""
                src={`/uploads/${params.placeid}-5.jpg`}
              />
              <img
                className="icon7"
                alt=""
                src={`/uploads/${params.placeid}-6.jpg`}
              />
            </div>
          </div>
        </div>
        <div className="place-reviews">
                ...
                <div className="review-card-slide">
                  <div className="review-card">
                    <div className="review-card-header">
                      <div style={{ marginTop: "2px", width: "145px" }}>
                        {review.name}
                      </div>
          ...
          <div className="review-card-write">
            ...
            >
              <Input
                className="createblog-dis-custom-inputx "
                maxLength={19}
                value={name}
